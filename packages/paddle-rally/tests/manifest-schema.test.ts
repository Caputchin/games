...
describe('paddle-rally caputchin.json - schema / presets parity', () => {
  ...
  it('declares exactly one default preset', () => {
    const defaults = presetNames.filter((n) => presets[n]?.['_default'] === true);
    expect(defaults, `exactly one _default preset, got ${JSON.stringify(defaults)}`).toHaveLength(1);
  });
  ...
  it('covers the official supported-language set', () => {
