...
- **What it does not claim:** Paddle Rally is not a Turing test. A program that genuinely tracks and returns the ball plays the way a person does and can pass, as it could for any skill-based challenge. A random/erratic bot can also luck through at a bounded, probabilistic rate. It is a filter, not a proof.
- **The target setting is a security dial.** Points to win (rival) and returns to survive (solo) run from 1 to 10, default 3. A longer game is a smaller luck window, so it filters random bots harder; a shorter one trades that resistance for a quicker check. At the default and above (3 or more) the random-bot pass rate stays low (a few percent, falling toward zero at the higher settings). The lowest settings (1, and to a lesser degree 2) are low-friction, low-security choices: a one-point rival or one-return solo lets a meaningful share of random bots through, so pick them only when speed matters more than strength and other Caputchin signals carry the load. Non-playing input (a still or held paddle) still loses at every setting.
