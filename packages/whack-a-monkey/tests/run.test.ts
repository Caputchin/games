...
describe('run artifact (toRun)', () => {
  it('passes with the replayed score when the trace taps enough monkeys', () => {
    const live = play(SEED, DEFAULT_SIM_CONFIG, {
      tapUntil: DEFAULT_SIM_CONFIG.passHits + 2,
      maxTicks: MAX,
    });
    const verdict = run(SEED, null, encodeTrace(live.recorded));
    expect(verdict.passed).toBe(true);
    expect(verdict.score).toBe(live.score);
    expect(verdict.score).toBeGreaterThanOrEqual(DEFAULT_SIM_CONFIG.passHits);
