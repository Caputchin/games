...
// else. No DOM, no rendering, no audio — `toRun` turns the pure reducer in
// sim/engine into the contract function.
//
// `passed` reads the gate threshold from `config` (server-supplied, safe),
// never from the trace. At MVP the server passes `null`, so DEFAULT_SIM_CONFIG
// is what the gate uses; per-site config injection is a deferred server-only
// phase.
...
export const run = toRun(engine, {
  ...
  maxTicks: MAX_TICKS,
  passed: (outcome, config) => outcome.score >= config.passHits,
});
