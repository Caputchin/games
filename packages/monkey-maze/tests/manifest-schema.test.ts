...
describe('monkey-maze caputchin.json - schema / presets parity', () => {
  ...
  it('every preset declares _lang and _default', () => {
    for (const name of presetNames) {
      expect(typeof presets[name]?.['_lang'] === 'string', `preset "${name}" _lang`).toBe(true);
      expect(presets[name]?.['_default'], `preset "${name}" _default`).toBe(true);
    }
  });
