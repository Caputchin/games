...
{ 
    ...
    // Phobos: spawn the seeded monster wave into the (monster-free) arena.
    { extern void phobos_spawn_wave(void); phobos_spawn_wave(); }
...
{ 
	...
	{ 
	    ...
	    // Phobos: capture (live) / inject (replay) the console player's cmd.
	    {
		extern int phobos_recording, phobos_replaying;
		extern void phobos_read_cmd(ticcmd_t *), phobos_write_cmd(ticcmd_t *);
		if (i == consoleplayer)
		{
		    if (phobos_replaying) phobos_read_cmd(cmd);
		    if (phobos_recording) phobos_write_cmd(cmd);
		}
	    }
...
void G_DoNewGame (void) 
{
    ...
    nomonsters = false;
    consoleplayer = 0;
    // Phobos: re-apply the server difficulty the reset above just cleared, so
    // G_InitNew reads the configured fast/respawn (identical in live + replay).
    {
	extern int phobos_have_seed, phobos_fast, phobos_respawn;
	if (phobos_have_seed)
	{
	    fastparm = phobos_fast ? true : false;
	    respawnparm = phobos_respawn ? true : false;
	}
    }
  ...
  int		map )
{
    ...
    */
    ...
    // Phobos: idempotent fast-monster setup. Vanilla DOOM mutates the global
    // states[]/mobjinfo[] speed tables in place and only reverts when
    // transitioning FROM nightmare, so toggling fastparm across rounds on a warm
    // replay isolate would leave the tables corrupted (a non-fast round would
    // run fast). Track the current applied state and apply only the delta, so
    // any sequence of fast/non-fast rounds is reproducible. Chocolate Doom's
    // G_SetFastParms is the same idea.
    {
	static int phobos_states_fast = 0;
	int want_fast = (fastparm || skill == sk_nightmare) ? 1 : 0;
	if (want_fast && !phobos_states_fast)
	{
	    ...
	    phobos_states_fast = 1;
	}
	else if (!want_fast && phobos_states_fast)
	{
	    for (i=S_SARG_RUN1 ; i<=S_SARG_PAIN2 ; i++)
		states[i].tics <<= 1;
