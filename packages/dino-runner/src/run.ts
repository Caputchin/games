...
// config->sim transform (`resolveSimConfig`, including the jumpVelocity sign
// bridge) and the pass decision (`engine.result` -> `state.verified`). The live
// driver runs the SAME `engine.init` over the SAME raw config, so the replayed
// verdict equals live play - no external transform or gate that one path could
// compute differently. `toRun` adds only the truncated guard (a non-terminating
