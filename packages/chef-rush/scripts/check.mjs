...
//
// This Excalibur engine game has no wasm to load, so it consumes the check
// library directly (like the Kaplay / Phaser games), importing the run artifact
// straight.
import { selfCheckRun } from '@caputchin/replay-selfcheck';
import { run } from '../dist/run.js';

// Write directly to stdout, not console.log: the Excalibur preset quiets its
// logger inside the run bundle, which would otherwise swallow this report
// (leaving a blank CI log) while the exit code still gates.
