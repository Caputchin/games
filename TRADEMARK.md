...
**Status:** "Caputchin" is a common-law trademark held through continuous use in commerce. Our enforcement layer is common-law trademark plus these guidelines. We can send cease-and-desist letters and file platform takedowns.
...
| GitHub repo name | Code identifier | `johndoe/caputchin-stripe-bridge` is fine |
| CLI binary name | Code identifier | `caputchin-deploy` (third-party tool) is fine |
| README title / hero text / marketing label | Marketed product name | "Caputchin Stripe Bridge" as a product brand: please ask; "stripe-bridge for Caputchin (third-party)" is fine |
...
When the same project has both (for example, npm package `caputchin-react-helper` whose README hero says "ReactCaputchinKit, a third-party helper for Caputchin"), the code identifier is fine; the README hero is a marketed name and should not lead with "Caputchin" as the product brand.
