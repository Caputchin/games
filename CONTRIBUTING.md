...
2. Write `packages/<game-id>/caputchin.json` with the [marketplace manifest](https://github.com/Caputchin/caputchin-platform/blob/main/docs/marketplace.md) fields.
...
- **Bundle is single-file**, IIFE format. No code splitting, no dynamic `import()`, no `Worker(url)`, no external `fetch`. All assets inline as data URLs. See [game-distribution](https://github.com/Caputchin/caputchin-platform/blob/main/docs/game-distribution.md#bundle-constraint).
